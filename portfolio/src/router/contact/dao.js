...
exports.list = (req,res) => {
	...
	sql = ` SELECT  count(*) cnt FROM ct_board WHERE board_code = ? ${where} `;
	conn.query(sql,[body.board_code],(err,data)=>{
		...
		sql = ` SELECT * FROM ct_board WHERE board_code = ? ${where} ORDER BY num DESC LIMIT ?, ? `; 
...
exports.add = (req,res) => {
	...
	sql = " INSERT INTO  tb_board (board_code, name, email, title, contents, regdate) values (?, ?, ?, ?, ?, now()) ";
	conn.query(sql,
		[body.board_code
		, body.name
		, body.email
...
exports.view = (req,res) => {
	body = req.query;
	num = req.params.num;
	sql = " SELECT * FROM ct_board WHERE board_code = ? AND num = ? ";
