  ...
  export default {
    ...
    methods: {
      fnGetList() { //데이터 가져오기 함수
			...
			this.$axios.get('http://localhost:8080/contact',{params:this.body})
			.then((res)=>{
				if(res.data.success) {
		...
		,getList() {
			this.$axios.get("http://localhost:8080/contact")
			.then((res)=>{
				console.log(res);
			})
			.then((err)=>{
...
<style scoped>
  #list{ width: inherit; height:1024px; position:relative; background-color: #89b2ce; }
  #llist-head{ width:inherit; border-bottom:1px #D8D8D8 solid; height:70px; position:fixed; z-index:10; top:0; left:50%; transform: translateX(-50%); background-color:white; color:black; text-align:center; line-height:70px; font-size:30px; font-weight:bold; opacity:0.8;}
